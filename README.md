...
### 结果页长这样（小红书实例，3 候选 grid 对比）

![小红书种草文案结果页](examples/screenshots/shot-xiaohongshu.png)
...
![客服机器人结果页](examples/screenshots/shot-cs-bot.png)

**合同要素抽取**（单候选 + schema 强制 + 19 项检查清单，含输出强制层）：

![合同抽取结果页](examples/screenshots/shot-contract.png)
...
```mermaid
flowchart TD
    A["用户一句话需求"] --> B{"prompt-architect 入口<br/>围栏防注入 · 复杂度筛查 · 路由"}
    B -->|要一张图| I["pa-image<br/>图像编译器（按厂商适配）"]
    B -->|要一段视频| V["pa-video<br/>视频编译器（分镜/音频/制式）"]
    B -->|文字 prompt| C["pa-deconstruct 意图挖掘<br/>产出 IR：体裁/风格/成功判据/输出契约"]
...
套件规则蒸馏自对 20 个 top prompt-engineering 框架（DSPy、guidance、outlines、instructor、LangGPT、BAML、guardrails、TextGrad、promptfoo、Prompty、LMQL、ell、mirascope、promptbase、12-factor-agents…）的源码级分析，完整知识库见 [`docs/prompt-engineering-knowledge.html`](docs/prompt-engineering-knowledge.html)。

MIT License · 欢迎 issue / PR。

*本 README 与三个测试实例均由该套件自身的流水线生成——吃自己的狗粮。*
